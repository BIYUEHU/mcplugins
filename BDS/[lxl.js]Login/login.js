const MainPath = "./plugins/biyuehu/login/";
const version = "1.0.0";
const logPath = "./logs/biyuehu/login/";
loading();
...
mc.regPlayerCmd("login", "/login <password> Login account", (pl, args) => login(pl, args), 0)
mc.regPlayerCmd("setpass", "/setpass <newpassword> <oldpassword> Set login password", (pl, args) => setpass(pl, args), 0)
mc.regPlayerCmd("quedata", "/quedata <playername> Query any data of someplayer", (pl, args) => quedata(pl, args), 1)
...
mc.listen("onJoin", (pl) => {
    ...
    if (datais == true) {    
        ...
        if (pl.xuid == xuid) {            
            ...
            if (pl.kick("§c当前Xuid与记录数据不匹配,请勿使用假名!") == true) {
                back = "玩家" + pl.name +" 的Xuid" + pl.xuid + " 与记录数据不匹配,已自动断开连接";
                logfile(back);
            }
        }
    } else {
        pl.tell("<§4Login§r>§g记录数据中无此Xuid，正在记录")
            ...
            network.httpGet("https://wanghun.top/api/yh/v2/ipip.php?ip=" + ip.split(":")[0], (status, result) => {
                if (status == 200) {
                ...
                if (File.writeTo(MainPath + "data/" + pl.name + ".json", datas) == true) {
    ...
    if (timedis == true) {
        ...
        tmsz = parseInt(tmss[4]) + parseInt(tmss[3]) * 60 + parseInt(tmss[2]) * 24 * 60 + parseInt(tmss[1]) * 30 * 24 * 60 + parseInt(tmss[0]) * 12 * 30 * 24 * 60;

        tm = system.getTimeObj();
        tmz = tm.m + tm.h * 60 + tm.D * 24 * 60 + tm.M * 30 * 24 * 60 + tm.Y * 12 * 30 * 24 * 60;
        ...
        if (lastm <= 3) {
            ...
            lastime = lastY + "年" + lastM + "月" + lastD + "天" + lasth + "小时" + lastm + "分钟前";
        }
        ...
        if (lastz >= loginint || hasTag != true) {
            ...
            setTimeout(() => {
                ...
                pl.tell("<§4Login§r>§c登录失效,请输入/login <密码>以登录游戏,否则" + logintime + "秒后将被踢出游戏");
                mc.runcmd("title \"" + pl.name + "\" title §c请先登录游戏")
                if (password == defpass) {
                    pl.tell("<§4Login§r>§c当前默认密码为" + data.fromBase64(defpass) + ",请输入/setpass <新密码> <旧密码(此时填默认密码)>以更改密码");
                }
                setTimeout(() => {
                    hasTag = pl.hasTag("logined");
                    if (hasTag != true) {
...
function loading () {
    ...
    if (ConfigIs != true) {
        ...
        File.writeTo(MainPath + "config.json", "{\"logintime\":60,\"loginint\":180,\"defpass\":\"114514\"}")

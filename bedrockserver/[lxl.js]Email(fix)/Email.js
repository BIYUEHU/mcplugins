...
function EmailMenu(pl) {
  pl.sendSimpleForm("§b邮件", "", EmailList, imagesw(EmailList), (pl, id) =>
    EmailBack(pl, id)
  );
}
...
function EmailBack(pl, id) {
  if (id != null) {
    ...
    if (id != Config.time.length && id != Config.time.length + 1) {
      ...
      pl.sendSimpleForm("§2草稿邮件", "暂无", ["返回"], [""], (pl) =>
        EmailMenu(pl)
      );
    } else if (id == Config.time.length + 1) {
      pl.sendSimpleForm("§1垃圾邮件", "暂无", ["返回"], [""], (pl) =>
        EmailMenu(pl)
      );
    }
  }
...
function loading() {
  ...
  if (ConfigIs != true) {
    ...
    File.writeTo(
      ...
      '{"title":["测试标题","一个邮件"],"time":["2022-1-21","2022-1-19"],"sender":["Server","某位不愿透露姓名的玩家"],"message":["这是一个测试的","22222"],"other":["minecraft:apple * 2\\nminecraft:iron_sword",""],"cmd":[["apple 2","iron_sword 1"],[]]}'
    );

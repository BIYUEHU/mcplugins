...
function addFavor(player, value) {
  ...
  if (player != null && player != {} && player != undefined) {
    if (
      File.exists(MainPath + "loverfavor/" + player.name + ".value") == true
    ) {
      File.writeTo(
        MainPath + "loverfavor/" + player.name + ".value",
        parseInt(
          File.readFrom(MainPath + "loverfavor/" + player.name + ".value")
        ) + value
      );
    } else {
...
function delFavor(player, value) {
  ...
  if (player != null && player != {} && player != undefined) {
    if (
      ...
      temp = parseInt(
        File.readFrom(MainPath + "loverfavor/" - player.name + ".value") - value
      );
      if (temp < 0) {
        temp = 0;
      }
...
mc.regPlayerCmd(
  "lover",
  "/lover <Many parameters> About lover or spouse",
  (pl, args) => lover(pl, args),
  0
);
mc.regPlayerCmd(
  "loveradmin",
  "/loveradmin <query|list|favor|add|del|rest|set> <playername> (<favorvalue>)",
  (pl, args) => loveradmin(pl, args),
  1
);
  ...
  if (true) {
    if (args[0] == "con") {
      ...
      if (args[1] != "" && args[1] != null && args[1] != undefined) {
        //2.一级参数con时，二级参数不能为空
        if (File.readFrom(MainPath + "lover/" + args[1]) == null) {
          //3.读取对方的lover/文件以检测对方状态
          if (args[1] != pl.name) {
            //4.检测告白玩家是否为自己
            var player = mc.getPlayer(args[1]);
            if (player != null && player != {} && player != undefined) {
              File.writeTo(
                MainPath + "lover/" + pl.name,
                `{"state": "confess","other": "${
                  player.name
                }","lovetime": ${JSON.stringify(nowtime())}}`
              );
              File.writeTo(
                MainPath + "lover/" + player.name,
                `{"state": "forconfess","other": "${
                  pl.name
                }","lovetime": ${JSON.stringify(nowtime())}}`
              );
              if (args[2] == "open") {
                chatf(
                  `<§cLover§r>§l§e玩家${pl.name}§r大声的对§b${player.name}§r说:${args[2]}`
                );
                player.tell(
                  `<§cLover§r>玩家§b${pl.name}§r大声的对你说:${args[2]}`
                );
              } else {
                pl.tell(`<§cLover§r>你悄悄的对§b${player.name}§r说:${args[2]}`);
                player.tell(
                  `<§cLover§r>玩家§b${pl.name}§r悄悄的对你说:${args[2]}`
                );
              }
              player.tell(
                "<§cLover§r>使用/lover accept 接受或/lover refuse 拒绝"
              );
            } else {
              pl.tell("§c无法获取玩家信息或不在线");
            }
      ...
      if (json != null && json != undefined) {
        ...
        if (state == "forconfess") {
          ...
          if (player != null && player != {} && player != undefined) {
            player.tell(
              `<§cLover§r>你的表白已被接受,现在你与玩家§b${pl.name}§r已成为情侣`
            );
          }
        } else {
          pl.tell("§c抛出异常");
          ...
          if (state == "lover") {
            ...
            if (player != null && player != {} && player != undefined) {
              ...
              File.writeTo(
                ...
                `{"state": "propose","other": "${
                  json.other
                }","protime": ${JSON.stringify(
                  nowtime()
                )},"lovetime":${JSON.stringify(lovetime)}}`
              );
              File.writeTo(
                MainPath + "lover/" + player.name,
                `{"state": "forpropose","other": "${
                  pl.name
                }","protime"1: ${JSON.stringify(
                  nowtime()
                )},"lovetime":${JSON.stringify(lovetime)}}`
              );

              chatf(
                `<§cLover§r>§l§e玩家${pl.name}§r大声的向§b${player.name}§r求婚`
              );
              player.tell(`<§cLover§r>玩家§b${pl.name}§r大声的向你求婚`);
              player.tell(
                "<§cLover§r>使用/lover accept2 接受或/lover refuse2 拒绝"
              );
            } else {
              pl.tell("§c无法获取玩家信息或不在线");
            }
      ...
      if (json != null && json != undefined) {
        ...
        if (state == "forpropose") {
          ...
          if (player != null && player != {} && player != undefined) {
            player.tell(
              `<§cLover§r>很遗憾,你的求婚遭到玩家§b${pl.name}§r的拒绝,关系破裂`
            );
          }
        } else {
          pl.tell("§c抛出异常");
      ...
      if (json != null && json != undefined) {
        ...
        if (
          ...
          if (player != null && player != {} && player != undefined) {
            ...
            moneytype == "llmoney"
              ? (havemoney = function () {
                  return moeny.get(pl.xuid);
                })
              : (havemoney = function () {
                  return pl.getScore(moneyname);
                });
            var back1 = function () {
              if (type == "item") {
                pl.tell(
                  `<§cLover§r>已向玩家§b${player.name}§r送出了 §3${gift.name}(${gift.id})`
                );
                player.tell(
                  `<§cLover§r>玩家§e${pl.name}对你送出了 §3${gift.name}(${gift.id})`
                );
              } else {
                pl.tell(
                  `<§cLover§r>已向玩家§b${player.name}§r送出了§g${args[4]}${moneyname}`
                );
                player.tell(
                  `<§cLover§r>玩家§e${pl.name}对你送出了 §g${args[4]}${moneyname}`
                );
              }
            };
            var back2 = function (type) {
              if (type == "item") {
                ...
                moneytype == "llmoney"
                  ? money.reduce(pl.xuid, parseInt(args[3])) &&
                    money.add(player.xuid, parseInt(args[3]))
                  : pl.reduceScore(scores, parseInt(args[3])) &&
                    player.addScore(scores, parseInt(args[3]));
              }
            };
            var back3 = function (type) {
              if (type == "item") {
                mc.runcmd(
                  `title @a title 玩家§b${player.name}§r收到了§e${pl.name}的礼物}`
                );
                chatf(
                  `§l<§cLover§r>@a 玩家§e${pl.name}对他/她的§c另一半 §b${player.xuid}§r送出了 §3${gift.name}(${gift.id})`
                );
              } else {
                mc.runcmd(
                  `title @a title 玩家§b${player.name}§r收到了§e${pl.name}的礼物}`
                ...
                chatf(
                  `§l<§cLover§r>@a 玩家§e${pl.name}对他/她的§c另一半 §b${player.xuid}§r送出了 §g${args[4]}${moneyname}`
                );
              }
            };
            //重复内容预设置
              ...
              if (
                ...
                if (args[1] == "open") {
                  ...
                  if (openmoney <= havemoney) {
                    moneytype == "llmoney"
                      ? money.reduce(pl.xuid, openmoney)
                      : pl.reduceScore(moneyname);
                    ...
                    pl.tell(
                      `<§cLover§r>余额不足无法缴纳公开送你手续费§g${openmoney} `
                    );
                  }
                } else {
                  back1("item");
            ...
            } else if (args[2] == "money") {
              //4.礼物类型 经济
              if (args[1] == "open") {
                //6.送礼是否公开 是则减公开送礼手续费
                if (
                  parseInt(args[3]) >= mingiftmoney &&
                  parseInt(args[3]) + openmoney <= havemoney
                ) {
                  moneytype == "llmoney"
                    ? money.reduce(pl.xuid, openmoney)
                    : pl.reduceScore(moneyname);
                  ...
                  pl.tell(
                    `<§cLover§r>你出的礼物金额低于服务器标准§g${mingiftmoney}${moneyname}§r或余额不足`
                  );
                }
              } else {
                if (
                  parseInt(args[3]) >= mingiftmoney &&
                  parseInt(args[3]) <= havemoney
                ) {
                  back1;
                  back2;
                } else {
      ...
      if (json != null && json != undefined) {
        ...
        if (
          ...
          if (
            ...
            player != {} &&
            player != undefined &&
            (json.other == "lover" || json.other == "spouse")
          ) {
      ...
      if (json != null && json != undefined) {
        ...
        if (
          ...
          var player = mc.getPlayer(json.other);
          player != null && player != {} && player != undefined
            ? mc.runcmd(`tp ${pl.name} ${player.name}`)
            : pl.tell("§c无法获取玩家信息或不在线");
        }
      } else {
        pl.tell("<§cLover§r>§6你没有对象");
      ...
      if (json != null && json != undefined) {
        ...
        other = json.other;
        lovetime =
          json.lovetime[0] + "-" + json.lovetime[1] + "-" + json.lovetime[2];
        protime = json.time;
        if (protime != null && protime != undefined) {
          protime =
            json.protime[0] + "-" + json.protime[1] + "-" + json.protime[2];
        } else {
          protime = "NULL";
        }
        pl.tell(
...
function loading() {
  ...
  log(
    `[HULI] 当我发布并且你看到这个插件时，说明我已经弃坑了噢~本插件代码最后更新:2022-3-5`
  );
